...
public class EmailService {
    ...
    public void sendBookingCancellationToUser(String to, Long bookingId) {
        String body = BOOKING_CANCELLED_BODY.replace(BOOKING_ID_PLACEHOLDER, String.valueOf(bookingId));
    ...
    public void sendBookingCancellationToConsultant(String to, Long bookingId) {
        String body = "<div style='font-family: Arial; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 10px;'>" +
                ...
                "<div style='padding: 30px;'><p>Booking <strong>#" + bookingId + "</strong> has been cancelled.</p><p>This time slot is now open and available for other clients to book.</p></div></div>";
    ...
    public void sendBookingReassignedToUser(String to, Long bookingId) {
        String body = "<div style='font-family: Arial; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 10px;'>" +
                "<div style='background-color: #3b82f6; color: white; padding: 20px; text-align: center;'><h1>Booking Rescheduled</h1></div>" +
                "<div style='padding: 30px;'><p>Your booking (<strong>#" + bookingId + "</strong>) has been updated by an Administrator.</p><p>Please log in to your dashboard to view your new Consultant and Time Slot details.</p></div></div>";
        sendEmail(to, "Booking Rescheduled - Meet the Masters", body);
    }
    ...
    public void sendBookingRemovedFromConsultant(String to, Long bookingId) {
        String body = "<div style='font-family: Arial; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 10px;'>" +
                ...
                "<div style='padding: 30px;'><p>Booking <strong>#" + bookingId + "</strong> has been reassigned to another consultant by an Administrator.</p><p>This slot is now available on your schedule again.</p></div></div>";
    ...
    public void sendSystemNotification(String to, String notificationMessage) {
        String body = SYSTEM_NOTIFICATION_BODY.replace("{message}", notificationMessage);
    ...
    public void sendTicketAssignedEmail(String to, String ticketNumber) {
        String body = TICKET_ASSIGNED_BODY.replace(TICKET_ID_PLACEHOLDER, ticketNumber);
        sendEmail(to, "New Ticket Assigned: #" + ticketNumber, body);
    }

    public void sendTicketCommentEmail(String to, String ticketNumber) {
        String body = TICKET_COMMENT_BODY.replace(TICKET_ID_PLACEHOLDER, ticketNumber);
