...
const ResetPasswordPage: React.FC<{ initialEmail?: string; onBackToLogin: () => void }> = ({ initialEmail = "", onBackToLogin }) => {
  ...
  const stepIndex = ["email", "otp"].indexOf(step);

  return (
    <div className="auth-page">
      <button onClick={onBackToLogin} className="glass-btn"
        style={{ position: "absolute", top: "clamp(12px, 4vw, 24px)", left: "clamp(12px, 4vw, 24px)", width: 42, height: 42, zIndex: 50 }} title="Back to Login">
        <ArrowLeft size={20} />
      </button>
      <div className="auth-card animate-fade-up">
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 3 }}>
            ...
            {step === "otp" && `Enter the OTP sent to ${email.trim().toLowerCase()} and your new password`}
        ...
        {step === "otp" && (
          <div className="auth-input-group">
            <div className="badge badge-info" style={{ marginBottom: 16, width: '100%', justifyContent: 'flex-start', padding: '10px 14px', textTransform: 'none' }}>
              <Mail size={16} />
              <span style={{ fontSize: 13, fontWeight: 700 }}>
                Check your inbox at <span style={{ fontSize: 11 }}>{email.trim().toLowerCase()}</span>
              </span>
            </div>
        ...
        {step === "done" && (
          <div className="success-overlay-static" style={{ textAlign: "center" }}>
            ...
            <div style={{ fontSize: 14, color: "var(--text-muted)", marginBottom: 28, lineHeight: 1.6 }}>
              OTP verified. Your password has been updated successfully.
            </div>
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!showTermsModal || termsLoaded) return;
  ...
  return (
    <div className="auth-page">
      ...
      <button onClick={() => navigate("/")} className="glass-btn"
        style={{ position: "absolute", top: "clamp(12px, 4vw, 24px)", left: "clamp(12px, 4vw, 24px)", width: 40, height: 40, zIndex: 50 }} title="Back to Home">
        <ArrowLeft size={20} />
      </button>
