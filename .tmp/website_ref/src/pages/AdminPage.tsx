...
const MAX_FLAT_COMMISSION = 100000;
const MAX_PERCENTAGE_VALUE = 100;
const MAX_OFFER_AMOUNT = 100000;

const prettifyEmailLocalPart = (email: string): string =>
  email
    .split("@")[0]
    .replace(/[._-]+/g, " ")
...
const resolveUserRoleLabel = (user: any): string => {
  const directRole = typeof user?.role === "string" ? user.role : typeof user?.userRole === "string" ? user.userRole : "";
  if (directRole) return directRole.toUpperCase().replace(/^ROLE_/, "");
  if (Array.isArray(user?.roles) && user.roles.length > 0) {
    const firstRole = user.roles[0];
    const roleName = typeof firstRole === "string" ? firstRole : firstRole?.authority || firstRole?.name || "";
...
const dedupeTicketsById = <T extends { id?: number | string }>(items: T[]) => {
...
const TICKET_STATUS_CFG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  ...
  RESOLVED: { label: "Resolved", color: "#16A34A", bg: "#F0FDF4", border: "#86EFAC" },
...
const SVGS = {
  ...
  phone: "M5 4h14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z M12 18h.01",
  ...
  return (
    <div style={{ padding: "14px 0 6px", position: "relative" }}>
      ...
      <div style={{
        ...
        height: 2, background: "#0F766E", zIndex: 1, transition: "width 0.4s ease",
      }} />
          ...
          return (
            <div key={step.key} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
              <div style={{
                ...
                background: done ? "#0F766E" : current ? "#ECFEFF" : "#F8FAFC",
                border: `2px solid ${done || current ? "#0F766E" : "#CBD5E1"}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 13, boxShadow: current ? "0 0 0 4px rgba(15,118,110,0.12)" : "none",
                ...
                {done
                  ...
                  : <span style={{ display: "flex", alignItems: "center", justifyContent: "center", color: current ? "#0F766E" : "#94A3B8" }}>{getStepSvg(step.key)}</span>}
              </div>
              <span style={{
                fontSize: 9, fontWeight: 700, textTransform: "uppercase",
                letterSpacing: "0.04em", color: done || current ? "#115E59" : "#94A3B8",
                textAlign: "center",
              }}>
                {step.label}
              </span>
...
const AssignConsultantModal: React.FC<AssignModalProps> = ({ ticket, consultants, onClose, onAssigned }) => {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      const consultantName = consultant?.name || (consultant as any)?.fullName || "Consultant";
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ background: "var(--portal-profile-gradient)", padding: "20px 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 10, color: "#99F6E4", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em" }}>
            ...
            <button onClick={onClose} style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", width: 32, height: 32, borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><X size={16} /></button>
          </div>
        </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: `2px solid ${isBlocked ? "#FCA5A5" : selected === c.id ? "#0F766E" : "#E2E8F0"}`,
                    background: isBlocked ? "#FEF2F2" : selected === c.id ? "#ECFEFF" : "#fff",
                    opacity: isBlocked ? 0.75 : 1,
                    transition: "all 0.15s",
                  }}
                >
                  <img src={c.avatar} alt={c.name} style={{ width: 40, height: 40, borderRadius: "50%", objectFit: "cover", border: `2px solid ${isBlocked ? "#FCA5A5" : "#A5F3FC"}` }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: isBlocked ? "#991B1B" : "#0F172A", display: "flex", alignItems: "center", gap: 8 }}>
                      {c.name}
                      {isBlocked && (
                        <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 6, background: "#FEE2E2", color: "#DC2626", fontWeight: 700, border: "1px solid #FECACA", display: "inline-flex", alignItems: "center", gap: 4 }}>
                          <SvgIcon d={SVGS.slash} size={11} color="#DC2626" strokeWidth={2.2} />
                          ESCALATED - BLOCKED
                        </span>
                      )}
                    </div>
                  ...
                  {selected === c.id && !isBlocked && (
                    <div style={{ width: 22, height: 22, borderRadius: "50%", background: "#0F766E", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
                    </div>
                  )}
                </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
            ...
            <button
              ...
              style={{
                ...
                background: (!selected || assigning) ? "#E2E8F0" : "var(--color-primary-gradient)",
...
const TicketDetailPanel: React.FC<TicketDetailProps> = ({
  ticket, consultants, currentAdminId, onClose, onStatusChange, onPriorityChange, onDeleted, onAssigned,
}) => {
  const { addNotification } = useNotifications();
  const ticketDisplayId = getTicketDisplayId(ticket);
  ...
  useEffect(() => {
    const timer = setInterval(async () => {
      try {
        ...
        if (!TERMINAL_TICKET_STATUSES.includes(localStatus) && fresh?.priority && fresh.priority !== localPriority) {
          setLocalPriority(fresh.priority as TicketPriority);
          onPriorityChange(ticket.id, fresh.priority as TicketPriority);
        }
  ...
  }, [ticket.id, localPriority, localStatus]);
    ...
    try {
      ...
      if (userId) {
        ...
        const notif = {
          ...
          title: `Ticket ${ticketDisplayId} ${cfg?.label || newStatus}`,
  ...
  const handlePriorityChange = async (newPriority: string) => {
    if (newPriority === localPriority || TERMINAL_TICKET_STATUSES.includes(localStatus)) return;
    const previousPriority = localPriority;
    setLocalPriority(newPriority as TicketPriority);
    onPriorityChange(ticket.id, newPriority as TicketPriority);
    setUpdatingPriority(true);
    try {
      // Try PATCH /tickets/:id/priority first, fallback to PATCH /tickets/:id
      try {
        await apiFetch(`/tickets/${ticket.id}/priority`, {
          method: "PATCH",
      ...
      onPriorityChange(ticket.id, previousPriority);
      showToast(e.message || "Priority update failed.", false);
    }
  ...
  const isPriorityLocked = TERMINAL_TICKET_STATUSES.includes(localStatus);

  const getUserLabel = () => {
    return resolveTicketClientName(ticket);
  };
  ...
  return (
    <>
      <ConfirmDialog
        ...
        title={`Delete Ticket ${ticketDisplayId}?`}
        message="This action cannot be undone. The ticket and its local admin context will be removed from the current view."
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 1200, display: "flex", alignItems: "stretch", justifyContent: "flex-end" }}>
        ...
        <div style={{
          ...
          {/* Header */}
          <div style={{ background: "var(--portal-profile-gradient)", padding: "20px 24px", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
              <div style={{ flex: 1, minWidth: 0, paddingRight: 12 }}>
                <div style={{ fontSize: 10, color: "#99F6E4", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 4 }}>
                  Ticket Number {ticketDisplayId}
                </div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
              ...
              <div style={{ position: "relative" }}>
                <button
                  ...
                  disabled={updatingPriority || isPriorityLocked}
                  title={isPriorityLocked ? "Priority is locked after the ticket is closed or resolved" : "Click to change priority"}
                  style={{
          ...
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "14px 24px", borderBottom: "1px solid #F1F5F9", background: "#FAFAFA" }}>
                ...
                {(["LOW", "MEDIUM", "HIGH", "URGENT", "CRITICAL"] as const).map(p => {
                  ...
                  return (
                    <button key={p} onClick={() => !isActive && handlePriorityChange(p)}
                      disabled={updatingPriority || isActive || isPriorityLocked}
                      style={{
                        padding: "5px 12px", borderRadius: 20, fontSize: 11, fontWeight: 700,
                        cursor: isActive ? "default" : "pointer",
                        background: isActive ? cfg.bg : "#fff",
              ...
              <div style={{ marginTop: 8, fontSize: 11, color: "#94A3B8", display: "flex", alignItems: "center", gap: 5 }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" /></svg>
                {isPriorityLocked ? "Priority is locked once the ticket is resolved, closed, or escalated." : "Priority change is reflected immediately in the ticket list."}
              </div>
            </div>
        ...
        <style>{`
          ...
          @keyframes mtmPulse {
            0%   { transform: scale(0.82); filter: blur(5px) drop-shadow(0 0 6px rgba(15,118,110,0.20));  opacity: 0.55; }
            50%  { transform: scale(1.12); filter: blur(0px) drop-shadow(0 0 26px rgba(15,118,110,0.65)); opacity: 1.00; }
            100% { transform: scale(0.82); filter: blur(5px) drop-shadow(0 0 6px rgba(15,118,110,0.20));  opacity: 0.55; }
          }
        `}</style>
      </div>
    </>
...
export const CreateTicketModal: React.FC<{
  ...
  const handleSubmit = async () => {
    if (!validateForm()) { setError("Please fix the highlighted ticket details."); return; }
...
const ExportDropdown: React.FC<ExportDropdownProps> = ({ tickets, label = "Export", compact = false }) => {
  ...
  const handleExcel = async () => {
    if (isSingle) {
      ...
      try { await exportTicketsExcel(); setStatus("done"); setStatusMsg(`${tickets.length} tickets exported to Excel`); }
      catch { await clientExportTicketsExcel(tickets); setStatus("done"); setStatusMsg(`${tickets.length} tickets exported to Excel`); }
    }
  };
  ...
  const handlePdf = async () => {
    if (isSingle) {
      ...
      try { await exportTicketsPdf(); setStatus("done"); setStatusMsg(`${tickets.length} tickets exported to PDF`); }
      catch { await clientExportTicketsPdf(tickets); setStatus("done"); setStatusMsg(`${tickets.length} tickets exported to PDF`); }
    }
  };
...
const TicketsSection: React.FC<TicketsSectionProps> = ({ consultants, currentAdminId, onTicketsLoaded }) => {
  ...
  const enrichTickets = async (arr: Ticket[]): Promise<Ticket[]> => {
    ...
    consultants.forEach((c: any) => {
      if (c.id) consultantLookup[Number(c.id)] = c.name || c.fullName || "Consultant";
    });
  ...
  useEffect(() => {
    ...
    const loadAllTicketsForSearch = async () => {
      setSearchLoading(true);
      try {
        const knownPages = totalPages > 0 ? totalPages : 1;
        const pages = Array.from({ length: knownPages }, (_, idx) => idx);
        const results = await Promise.all(
          pages.map(async (page) => {
            if (pageCache[page]) return { page, tickets: pageCache[page] };
            const result = await getTicketsPage(page, TICKET_PAGE_SIZE);
            const arr: Ticket[] = Array.isArray(result.content) ? result.content : extractArray(result.content);
            const enriched = await enrichTickets(arr);
            return { page, tickets: enriched };
          })
        );
        if (cancelled) return;
        setPageCache(prev => {
          const next = { ...prev };
          results.forEach(({ page, tickets }) => { next[page] = tickets; });
          return next;
        });
        setSearchPool(dedupeTicketsById(results.flatMap(result => result.tickets)));
      } catch {
        if (!cancelled) {
          setSearchPool(dedupeTicketsById([...tickets, ...Object.values(pageCache).flat()]));
        }
      } finally {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hasSearch, searchPool, totalPages]);

  const handleStatusChange = (id: number, status: string) => {
    const updateFn = (t: Ticket) => t.id === id ? { ...t, status: status as TicketStatus } : t;
    ...
    setSearchPool(prev => prev ? prev.map(updateFn) : prev);
  ...
  const handleDeleted = (id: number) => {
    ...
    setPageCache(prev => {
      const updated: Record<number, Ticket[]> = {};
      Object.entries(prev).forEach(([k, v]) => { updated[Number(k)] = v.filter(t => t.id !== id); });
      return updated;
    });
    setSearchPool(prev => prev ? prev.filter(t => t.id !== id) : prev);
    setSelectedTicket(null);
  };

  const handlePriorityChange = (id: number, priority: TicketPriority) => {
    const updateFn = (t: Ticket) => t.id === id ? { ...t, priority } : t;
    setTickets(prev => prev.map(updateFn));
    setPageCache(prev => {
      const updated: Record<number, Ticket[]> = {};
      Object.entries(prev).forEach(([k, v]) => { updated[Number(k)] = v.map(updateFn); });
  ...
  const getUserDisplay = (t: Ticket) => {
    return resolveTicketClientName(t);
  };

  const getConsultantDisplay = (t: Ticket) => {
    return t.consultantName || t.agentName ||
      (t as any).assignedTo?.name || (t as any).consultant?.name ||
      (t.consultantId ? (consultants.find((c: any) => Number(c.id) === Number(t.consultantId))?.name || null) : null) ||
      "";
  };

  const matchesTicketSearch = (t: Ticket, query: string) => {
    ...
    const haystacks = [
      ...
      t.category ?? "",
      getUserDisplay(t),
      getConsultantDisplay(t),
      t.status ?? "",
      TICKET_STATUS_CFG[t.status]?.label ?? "",
      t.priority ?? "",
      TICKET_PRIORITY_CFG[t.priority]?.label ?? "",
    ]
  ...
  const searchSource = hasSearch
    ? dedupeTicketsById([...(searchPool ?? []), ...tickets, ...Object.values(pageCache).flat()])
    : tickets;

  const visible = searchSource.filter(t => {
    if (filterStatus !== "ALL" && t.status !== filterStatus) return false;
    if (filterPriority !== "ALL" && t.priority !== filterPriority) return false;
    if (hasSearch && !matchesTicketSearch(t, searchQ)) return false;
    return true;
  });
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const }}>
          <button onClick={() => setShowCreate(true)}
            style={{ padding: "8px 16px", background: "#0F766E", border: "none", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
            + New Ticket
          </button>
          <ExportDropdown
          ...
          <button onClick={load} disabled={loading}
            ...
            {loading
              ...
              : <SvgIcon d={SVGS.refresh} size={13} color="currentColor" strokeWidth={2.2} />}
            Refresh
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 10, marginBottom: 20, overflowX: "auto" }}>
        {[
          { label: "Total", value: tickets.length, color: "#0F766E", bg: "#ECFEFF" },
          ...
          return (
            <button key={f} onClick={() => { setFilterStatus(f as any); setTicketPage(0); setPageCache({}); }}
              style={{
                ...
                borderColor: isActive ? (cfg?.color ?? "#0F766E") : "#E2E8F0",
                background: isActive ? (cfg?.bg ?? "#ECFEFF") : "#fff",
                color: isActive ? (cfg?.color ?? "#0F766E") : "#64748B",
                fontSize: 11, fontWeight: isActive ? 700 : 500, cursor: "pointer", transition: "all 0.15s",
                display: "flex", alignItems: "center", gap: 5,
              }}>
              {f !== "ALL" && isActive && <span style={{ width: 6, height: 6, borderRadius: "50%", background: cfg?.color ?? "#0F766E", flexShrink: 0 }} />}
          ...
          <div style={{ background: "#fff", border: "1px solid #F1F5F9", borderRadius: 16, overflowX: "auto", overflowY: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
            <div style={{ minWidth: 980 }}>
              <div style={{ display: "grid", gridTemplateColumns: ticketTableColumns, columnGap: 10, padding: "10px 18px", background: "#F8FAFC", borderBottom: "1px solid #F1F5F9", fontSize: 10, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.06em", alignItems: "center" }}>
                <div style={{ paddingLeft: 2 }}>Ticket Number</div><div>TITLE / CLIENT</div><div>CATEGORY</div><div>Priority</div>
                ...
                return (
                  <div key={ticket.id}
                            ...
                            : sla.warning
                              ? (
                                <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                                  <SvgIcon d={SVGS.warning} size={10} color="#D97706" strokeWidth={2.2} />
                                  {sla.label}
                                </span>
                              )
                              : `${sla.label}`}
                    ...
                    <div style={{ justifySelf: "start" }}>
                      <span
                        title={pc.label}
                        style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: 74, fontSize: 11, padding: "3px 8px", borderRadius: 6, background: pc.bg, color: pc.color, fontWeight: 800, cursor: "default", letterSpacing: "0.02em", whiteSpace: "nowrap" }}
                      >{pc.label}</span>
                    </div>
                    <div style={{ fontSize: 11, color: "#475569", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0 }}>
                      {(() => {
                        const cname = getConsultantDisplay(ticket);
                        return cname
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-start", minWidth: 0 }}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, padding: "3px 9px", borderRadius: 6, background: sc.bg, color: sc.color, border: `1px solid ${sc.border}`, fontWeight: 700, whiteSpace: "nowrap", lineHeight: "1.4" }}>{sc.label}</span>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 20, flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  <button onClick={() => goToPage(ticketPage - 1)} disabled={ticketPage === 0}
                    style={{ padding: "6px 14px", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: ticketPage === 0 ? "not-allowed" : "pointer", border: "1.5px solid #E2E8F0", background: ticketPage === 0 ? "#F8FAFC" : "#fff", color: ticketPage === 0 ? "#CBD5E1" : "#0F766E", transition: "all 0.15s", display: "inline-flex", alignItems: "center", gap: 6 }}
                    ...
                    pg === "…" ? (
                      ...
                      <button key={pg} onClick={() => goToPage(pg as number)}
                        style={{
                          ...
                          border: pg === ticketPage ? "2px solid #0F766E" : pageCache[pg as number] ? "1.5px solid #A5F3FC" : "1.5px solid #E2E8F0",
                          background: pg === ticketPage ? "#0F766E" : pageCache[pg as number] ? "#ECFEFF" : "#fff",
                          color: pg === ticketPage ? "#fff" : pageCache[pg as number] ? "#0F766E" : "#374151",
                          transition: "all 0.15s",
                        }}
                  ...
                  <button onClick={() => goToPage(ticketPage + 1)} disabled={ticketPage >= totalPages - 1}
                    style={{ padding: "6px 14px", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: ticketPage >= totalPages - 1 ? "not-allowed" : "pointer", border: "1.5px solid #E2E8F0", background: ticketPage >= totalPages - 1 ? "#F8FAFC" : "#fff", color: ticketPage >= totalPages - 1 ? "#CBD5E1" : "#0F766E", transition: "all 0.15s", display: "inline-flex", alignItems: "center", gap: 6 }}
...
const SettingsPage: React.FC<{ adminId: number; onLogout: () => void }> = ({ adminId, onLogout }) => {
  ...
  const getPasswordStrength = (p: string): { label: string; color: string; pct: number } => {
    ...
    if (score <= 3) return { label: "Good", color: "#0F766E", pct: 70 };
  ...
  const sectionBtnStyle = (active: boolean): React.CSSProperties => ({
    ...
    padding: "16px 20px", border: "none", background: active ? "#ECFEFF" : "#fff",
    cursor: "pointer", fontSize: 14, fontWeight: active ? 700 : 500,
    color: active ? "#0F766E" : "#0F172A", fontFamily: "inherit",
  ...
  const Toggle: React.FC<{ checked: boolean; onChange: (v: boolean) => void; label: string; sub?: string }> = ({ checked, onChange, label, sub }) => (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #F8FAFC" }}>
      ...
      <label style={{ position: "relative", display: "inline-block", width: 44, height: 24, cursor: "pointer", flexShrink: 0 }}>
        ...
        <span style={{ position: "absolute", inset: 0, background: checked ? "#0F766E" : "#CBD5E1", borderRadius: 24, transition: "0.2s" }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{
                  ...
                  background: activeTab === tab.id ? "#ECFEFF" : "#F8FAFC",
                  border: `1.5px solid ${activeTab === tab.id ? "#A5F3FC" : "#E2E8F0"}`,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  transition: "all 0.15s",
                }}>
                ...
                <div style={{ textAlign: "left" }}>
                  <div style={{ fontSize: 14, fontWeight: activeTab === tab.id ? 700 : 600, color: activeTab === tab.id ? "#0F766E" : "#0F172A" }}>
                    {tab.label}
                  </div>
              ...
              <span style={{
                fontSize: 18, color: activeTab === tab.id ? "#0F766E" : "#CBD5E1",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 24, padding: "16px 18px", background: "#fff", borderRadius: 14, border: "1px solid #E2E8F0" }}>
                  <div style={{ position: "relative" }}>
                    <div style={{
                      ...
                      background: avatarPreview ? "transparent" : "var(--portal-profile-gradient)",
                      display: "flex", alignItems: "center", justifyContent: "center",
                ...
                <div style={{ maxWidth: 480 }}>
                  ...
                  <div style={{ marginBottom: 14 }}>
                    ...
                    {secForm.confirm && secForm.confirm !== secForm.newPass && (
                      <div style={{ fontSize: 11, color: "#DC2626", fontWeight: 600, marginTop: 4, display: "inline-flex", alignItems: "center", gap: 4 }}>
                        <SvgIcon d={SVGS.warning} size={11} color="#DC2626" strokeWidth={2.2} />
...
const sc_styles: Record<string, React.CSSProperties> = {
  ...
  filterPillActive: { borderColor: "#0F766E", background: "#0F766E", color: "#fff" },
  primaryBtn: { padding: "9px 18px", borderRadius: 10, border: "none", background: "#0F766E", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" },
    ...
    <div style={sc_styles.panelWrap}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20 }}>
          ...
          {filtered.map(t => {
            ...
            return (
              <div key={t.id} onClick={() => setSelTicket(selected ? null : t)}
                style={{ ...sc_styles.ticketRow, borderLeft: `4px solid ${breached ? "#DC2626" : tsc.color}`, background: selected ? "#ECFEFF" : breached ? "#FFF8F8" : "#fff", outline: selected ? "2px solid #0F766E" : "none" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ flexShrink: 0 }}>
                  ...
                  {selected && <div style={{ fontSize: 10, color: "#0F766E", fontWeight: 700, marginTop: 4, display: "inline-flex", alignItems: "center", gap: 4 }}><ChevronLeft size={11} /> Click agent</div>}
                </div>
              </div>
            );
          })}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {selTicket && <div style={{ fontSize: 11, color: "#0F766E", textAlign: "center", fontWeight: 600, padding: "8px 0" }}>Click agent to assign Ticket {getTicketDisplayId(selTicket)}</div>}
        </div>
      </div>
...
const CannedResponses: React.FC<{}> = () => {
  ...
  const save = async () => {
    ...
    if (!form.body.trim()) newErrors.body = "Body text is required";
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{
            ...
            {/* Modal header */}
            <div style={{ background: "var(--portal-profile-gradient)", padding: "20px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>New Canned Response</div>
                  <div style={{ fontSize: 11, color: "#99F6E4" }}>Predefined reply for quick use in tickets</div>
                </div>
              </div>
              <button onClick={closeModal} style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", width: 30, height: 30, borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><X size={15} /></button>
            </div>
            {/* Modal body */}
            <div style={{ padding: "24px" }}>
              ...
              <input
                value={form.title}
                onChange={e => { setForm({ ...form, title: e.target.value }); if (formErrors.title) setFormErrors(p => ({ ...p, title: undefined })); }}
          ...
          <div key={r.id} style={{ ...sc_styles.cannedCard, cursor: "default" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 11, background: "#ECFEFF", color: "#0F766E", padding: "2px 8px", borderRadius: 6, fontWeight: 600 }}>{r.category}</span>
              </div>
...
interface TicketCategory { id: number; name: string; localOnly?: boolean; }
...
const CategoriesConfig: React.FC<{}> = () => {
  ...
  const deleteCat = async (cat: TicketCategory) => {
    if (!cat.localOnly) return;
    try {
      await deleteTicketCategory(cat.id);
      setCats((prev) => prev.filter((item) => item.id !== cat.id));
  ...
  const addCat = async () => {
    ...
    try {
      ...
      setCats((prev) => [...prev, { id: created.id ?? Date.now(), name: created.name || name, localOnly: true }].sort((a, b) => a.name.localeCompare(b.name)));
      setNewCat({ name: "" });
  ...
  return (
    <div style={sc_styles.panelWrap}>
      <ConfirmDialog
        ...
        message={`"${deleteTarget?.name || ""}" will be removed from the admin-configured category list.`}
      ...
      <div style={sc_styles.panelHeader}>
        <div>
          <h3 style={sc_styles.panelTitle}>Ticket Categories</h3>
          <p style={sc_styles.panelSub}>Manage the category list used in the support ticket forms.</p>
        </div>
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid #F1F5F9", borderRadius: 14, overflow: "hidden", marginBottom: 16 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 140px 90px", padding: "10px 16px", background: "#F8FAFC", fontSize: 10, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "0.06em" }}>
          <div>Category</div><div>Source</div><div></div>
        </div>
        {loading ? <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}><img src={logoImg} alt="Meet The Masters" style={{ width: 48, height: "auto", animation: "mtmPulse 1.8s ease-in-out infinite" }} /></div>
          : cats.length === 0 ? (
            <div style={{ padding: "24px 16px", fontSize: 13, color: "#94A3B8", textAlign: "center" }}>No categories available yet.</div>
          )
            : cats.map((c, i) => (
              <div key={c.id} style={{ display: "grid", gridTemplateColumns: "1fr 140px 90px", padding: "12px 16px", borderTop: i > 0 ? "1px solid #F8FAFC" : "none", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: "#0F172A" }}>{c.name}</span>
                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: "fit-content", padding: "4px 10px", borderRadius: 999, border: "1px solid #E2E8F0", background: "#F8FAFC", color: "#475569", fontSize: 11, fontWeight: 600 }}>
                  {c.localOnly ? "Admin added" : "Unique categories"}
                </span>
                {c.localOnly ? (
                  <button onClick={() => setDeleteTarget(c)} style={{ ...sc_styles.iconBtn, color: "#64748B", borderColor: "#E2E8F0" }}>
...
const ReportsAnalytics: React.FC<{ tickets: Ticket[] }> = ({ tickets }) => {
  ...
  const kpis = [
    { label: "Total Tickets", value: total, color: "#0F766E", icon: "ticket" },
        ...
        <div style={sc_styles.chartCard}>
          ...
          {Object.entries(catCounts).sort((a, b) => b[1] - a[1]).map(([cat, count]) => (
            <div key={cat} style={{ marginBottom: 10 }}>
              ...
              <MiniBar val={count} max={catMax} color="#0F766E" />
            </div>
          ))}
...
const AddMemberPanel: React.FC = () => {
  ...
  const [addedMembers, setAddedMembers] = useState<{ id: number; name: string; email: string; role: string; addedAt: string; mobileNumber?: string }[]>([]);
  const [loadingAddedMembers, setLoadingAddedMembers] = useState(true);

  const showToast = (msg: string, ok = true) => { setToast({ msg, ok }); setTimeout(() => setToast(null), 3500); };

  const loadRecentMembers = async () => {
    setLoadingAddedMembers(true);
    try {
      const data = await apiFetch("/users");
      const users = extractArray(data);
      const memberRoles = new Set(["USER", "MEMBER", "CLIENT", "SUBSCRIBER", "SUBSCRIBED", "GUEST"]);
      const mapped = users
        .flatMap((user: any) => {
        ...
        .sort((a, b) => {
          ...
          const aDate = Date.parse(a.addedAt || "");
  ...
  const validate = () => {
    ...
    if (!normalizedName) e.name = "Full name is required";
    else if (normalizedName.length < 2) e.name = "Enter a valid full name";

    if (!normalizedEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)) e.email = "Valid email required";
    else if (addedMembers.some((member) => canonicalTextKey(member.email) === canonicalTextKey(normalizedEmail))) e.email = "This email already exists";

    if (!normalizedMobile || !/^[6-9]\d{9}$/.test(normalizedMobile)) e.mobileNumber = "Valid 10-digit mobile required";
    else if (addedMembers.some((member) => String(member.mobileNumber || "") === normalizedMobile)) e.mobileNumber = "This mobile number already exists";

    if (normalizedLocation && normalizedLocation.length < 2) e.location = "Enter a valid location";
    setErrors(e);
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      setAddedMembers((prev) => [newMember, ...prev].slice(0, 10));
      loadRecentMembers();
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px 18px" }}>
            ...
            <div>
              <label style={labelStyle}>Email Address *</label>
              <input value={form.email} onChange={e => { setForm(f => ({ ...f, email: e.target.value.trimStart().toLowerCase() })); setErrors(x => ({ ...x, email: "" })); }} placeholder="member@example.com" type="email" style={inputStyle} />
            ...
            <div>
              ...
              <div style={{ display: "flex" }}>
                ...
                <input value={form.mobileNumber}
                  onChange={e => { setForm(f => ({ ...f, mobileNumber: e.target.value.replace(/\D/g, "").slice(0, 10) })); setErrors(x => ({ ...x, mobileNumber: "" })); }}
          ...
          {/* Simple info */}
          <div style={{ background: "#ECFEFF", border: "1px solid #A5F3FC", borderRadius: 10, padding: "12px 16px", marginTop: 16, fontSize: 12, color: "#115E59", lineHeight: 1.6 }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 14, overflow: "hidden" }}>
          ...
          {loadingAddedMembers ? (
            <div style={{ padding: "24px 16px", textAlign: "center" }}>
              <img src={logoImg} alt="" style={{ width: 36, height: "auto", animation: "mtmPulse 1.8s ease-in-out infinite" }} />
            </div>
              ...
              {addedMembers.map((m, i) => {
                return (
                  <div key={m.id} style={{ padding: "12px 16px", borderBottom: i < addedMembers.length - 1 ? "1px solid #F1F5F9" : "none" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 34, height: 34, borderRadius: "50%", background: "var(--portal-profile-gradient)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
    ...
    <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 14, padding: 24, flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div>
        ...
        <textarea
          ...
          rows={6}
          style={{ width: "100%", border: "1.5px solid #CBD5E1", borderRadius: 12, padding: "14px 16px", fontSize: 14, fontFamily: "inherit", resize: "vertical", outline: "none", boxSizing: "border-box", opacity: enabled ? 1 : 0.5, minHeight: 140, lineHeight: 1.7, background: enabled ? "#FFFFFF" : "#F8FAFC" }}
        ...
        <div style={{ display: "flex", borderBottom: "1px solid #E2E8F0", overflowX: "auto" }}>
          {SUPPORT_CONFIG_TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{ padding: "13px 20px", border: "none", background: "none", cursor: "pointer", fontSize: 13, fontWeight: tab === t.id ? 700 : 500, color: tab === t.id ? "#0F766E" : "#64748B", borderBottom: tab === t.id ? "2px solid #0F766E" : "2px solid transparent", display: "flex", alignItems: "center", gap: 7, transition: "all 0.15s", whiteSpace: "nowrap", fontFamily: "inherit" }}>
              <span style={{ color: tab === t.id ? "#0F766E" : "#94A3B8", display: "flex", alignItems: "center" }}>{getConfigTabSvg(t.id, tab === t.id)}</span>{t.label}
            </button>
          ))}
        </div>
        <div style={{ padding: "24px" }}>
...
const AdminBookingsPanel: React.FC<{
  ...
  const statusColor: Record<string, { color: string; bg: string; border: string }> = {
    ...
    COMPLETED: { color: "#0F766E", bg: "#ECFEFF", border: "#99F6E4" },
...
const CommissionConfigPanel: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (feeConfig.feeType === "PERCENTAGE" && val > MAX_PERCENTAGE_VALUE) { showToast(`Percentage cannot exceed ${MAX_PERCENTAGE_VALUE}%.`); return; }
    if (feeConfig.feeType === "FLAT" && val > MAX_FLAT_COMMISSION) { showToast(`Fixed commission cannot exceed ${formatIndianCurrency(MAX_FLAT_COMMISSION)}.`); return; }
    setSaving(true);
    try {
        ...
        <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 16, padding: 28, maxWidth: 540, boxShadow: "0 2px 10px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              {(["FLAT", "PERCENTAGE"] as const).map(type => (
                <button key={type} onClick={() => setFeeConfig(f => ({ ...f, feeType: type }))}
                  style={{ flex: 1, padding: "11px", borderRadius: 10, border: `2px solid ${feeConfig.feeType === type ? "#0F766E" : "#E2E8F0"}`, background: feeConfig.feeType === type ? "#ECFEFF" : "#fff", color: feeConfig.feeType === type ? "#0F766E" : "#64748B", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          ...
          <div style={{ marginBottom: 22 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input
                ...
                onChange={e => setFeeConfig(f => ({
                  ...f,
                  feeValue: feeConfig.feeType === "PERCENTAGE"
                    ? sanitizeDecimalInput(e.target.value, MAX_PERCENTAGE_VALUE, 2)
                    : sanitizeDecimalInput(e.target.value, MAX_FLAT_COMMISSION, 2),
                }))}
                placeholder={feeConfig.feeType === "PERCENTAGE" ? `e.g. 15 (max ${MAX_PERCENTAGE_VALUE})` : `e.g. 200 (max ${formatIndianNumber(MAX_FLAT_COMMISSION)})`}
                type="text"
                inputMode="decimal"
          ...
          <div style={{ marginBottom: 22 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
              ...
              <input
                value={preview ?? 1000}
                onChange={e => setPreview(Number(sanitizeWholeNumberInput(e.target.value, MAX_FLAT_COMMISSION)) || 0)}
                type="text"
                inputMode="numeric"
                style={{ width: 120, padding: "6px 10px", border: "1.5px solid #E2E8F0", borderRadius: 8, fontSize: 13, outline: "none" }}
              />
            ...
            <div style={{ background: "#F0FDF4", border: "1px solid #86EFAC", borderRadius: 10, padding: "12px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 4 }}>
                ...
                <span style={{ fontWeight: 600 }}>{formatIndianCurrency(exampleBase)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 4 }}>
                <span style={{ color: "#475569" }}>Platform Commission</span>
                <span style={{ fontWeight: 600, color: "#0F766E" }}>{formatIndianCurrency(exCommission)}</span>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}>
                ...
                <span style={{ fontWeight: 800, color: "#166534" }}>{formatIndianCurrency(exTotal)}</span>
              </div>
            </div>
          </div>
          <button onClick={handleSave} disabled={saving}
...
const QuestionsManagementPanel: React.FC = () => {
  ...
  const [deleteSkillTarget, setDeleteSkillTarget] = React.useState<SkillItem | null>(null);
  ...
  const [deleteQuestionTarget, setDeleteQuestionTarget] = React.useState<QItem | null>(null);
  ...
  const load = async () => {
    ...
    } catch (e: any) { console.warn("Load failed:", e?.message); setSkills([]); setQuestions([]); }
    finally { setLoading(false); }
  };
  ...
  const renderQuestionTypeIcon = (type: QType, color: string, size = 14) => {
    ...
    if (type === "mobile") return <SvgIcon d={SVGS.phone} size={size} color={color} strokeWidth={2} />;
    return <SvgIcon d={SVGS.pencil} size={size} color={color} strokeWidth={2} />;
  };
  ...
  const handleSaveQ = async () => {
    ...
    if (qForm.type === "radio" || qForm.type === "multiselect") {
      const options = parseOptions(qForm.optionsRaw);
      if (options.length < 2) { showToast("Please add at least 2 options (one per line)."); return; }
      if (new Set(options.map(canonicalTextKey)).size !== options.length) { showToast("Duplicate options are not allowed."); return; }
    }
    setSavingQ(true);
    try {
  ...
  const TYPE_CFG: Record<QType, { label: string; iconKey: QType; color: string; bg: string; border: string }> = {
    radio: { label: "Single choice", iconKey: "radio", color: "#0F766E", bg: "#ECFEFF", border: "#A5F3FC" },
    multiselect: { label: "Multi-select", iconKey: "multiselect", color: "#7C3AED", bg: "#F5F3FF", border: "#DDD6FE" },
  ...
  return (
    <div>
      <ConfirmDialog
        ...
        title="Delete Skill?"
        message={`"${deleteSkillTarget?.name || ""}" will be removed. Consultant tags linked to it may be affected.`}
        confirmLabel="Delete Skill"
        danger={true}
        busy={deleteSkillTarget ? deletingSkill === deleteSkillTarget.id : false}
        onClose={() => { if (deletingSkill === null) setDeleteSkillTarget(null); }}
        onConfirm={() => {
          if (!deleteSkillTarget?.id) return;
          handleDeleteSkill(deleteSkillTarget.id);
      ...
      <ConfirmDialog
        ...
        message={`"${deleteQuestionTarget?.text || ""}" will be removed along with any linked client answers.`}
        confirmLabel="Delete Question"
        danger={true}
        busy={deleteQuestionTarget ? deletingQ === deleteQuestionTarget.id : false}
        onClose={() => { if (deletingQ === null) setDeleteQuestionTarget(null); }}
        onConfirm={() => {
          if (!deleteQuestionTarget?.id) return;
          handleDeleteQ(deleteQuestionTarget.id);
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        ...
        <button onClick={panelTab === "skills" ? openNewSkill : openNewQ}
          style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 18px", background: "var(--color-primary-gradient)", color: "#fff", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
        ...
        <div>
          ...
          {skills.length === 0 && !showSkillForm ? (
            <div style={{ textAlign: "center", padding: "60px 20px", background: "#F8FAFC", borderRadius: 16, color: "#94A3B8" }}>
              ...
              <button onClick={openNewSkill} style={{ padding: "10px 22px", background: "#0F766E", color: "#fff", border: "none", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Create First Skill</button>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {skills.map(skill => (
        ...
        <div>
                  ...
                  {(["radio", "multiselect", "text", "mobile"] as QType[]).map(t => {
                    ...
                    return (
                      <button key={t} onClick={() => setQForm(f => ({ ...f, type: t }))}
                        ...
                        <span style={{ display: "inline-flex", alignItems: "center" }}>{renderQuestionTypeIcon(cfg.iconKey, sel ? cfg.color : "#64748B")}</span>
                        {cfg.label}
                      </button>
                    );
                  })}
          ...
          {questions.length === 0 && !showQForm ? (
            <div style={{ textAlign: "center", padding: "60px 20px", background: "#F8FAFC", borderRadius: 16, color: "#94A3B8" }}>
              ...
              <button onClick={openNewQ} style={{ padding: "10px 22px", background: "#0F766E", color: "#fff", border: "none", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Add First Question</button>
            </div>
          ) : (
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}`, display: "inline-flex", alignItems: "center", gap: 4 }}>
                            {renderQuestionTypeIcon(cfg.iconKey, cfg.color, 12)} {cfg.label}
                          </span>
                          {q.type === "mobile" && (
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                            {q.options.slice(0, 6).map((opt, i) => (
                              <span key={i} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 20, background: "#F8FAFC", color: "#475569", border: "1px solid #E2E8F0", fontWeight: 500 }}>
                                <span style={{ marginRight: 4, display: "inline-flex", verticalAlign: "text-bottom" }}>
                                  {renderQuestionTypeIcon(q.type === "radio" ? "radio" : "multiselect", "#64748B", 10)}
                                </span>{opt}
                              </span>
                            ))}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {(["ALL", "PENDING", "APPROVED", "REJECTED"] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: "7px 16px", borderRadius: 20, border: `1.5px solid ${filter === f ? "#0F766E" : "#E2E8F0"}`, background: filter === f ? "#ECFEFF" : "#fff", color: filter === f ? "#0F766E" : "#64748B", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
...
const AdminMasterTimeSlotsPanel: React.FC = () => {
  ...
  const [selectedDurationHours, setSelectedDurationHours] = React.useState(1);
  const [editingId, setEditingId] = React.useState<number | null>(null);
  const [editingStartHour, setEditingStartHour] = React.useState<number | null>(null);
  const [editingDurationHours, setEditingDurationHours] = React.useState(1);
  const [saving, setSaving] = React.useState(false);
  ...
  const handleCreate = async () => {
    ...
    const nextRange = formatHourRangeLabel(selectedStartHour, selectedDurationHours);
    if (slots.some((slot) => slot.timeRange.toLowerCase() === nextRange.toLowerCase())) {
      return showToast("This time slot already exists.");
    }
    setSaving(true);
    try {
      await apiFetch("/master-timeslots", {
        method: "POST",
        body: JSON.stringify({ timeRange: nextRange, duration: durationHoursToMinutes(selectedDurationHours, 1) }),
      });
  ...
  const handleUpdate = async (id: number) => {
    ...
    const nextRange = formatHourRangeLabel(editingStartHour, editingDurationHours);
    ...
    try {
      await apiFetch(`/master-timeslots/${id}`, {
        method: "PUT",
        body: JSON.stringify({ timeRange: nextRange, duration: durationHoursToMinutes(editingDurationHours, 1) }),
      });
      setEditingId(null);
      setEditingStartHour(null);
      setEditingDurationHours(1);
      await load();
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      {deleteTargetId !== null && (
        <ConfirmDialog
          ...
          message="Delete this time range?"
          confirmLabel={deletingId === deleteTargetId ? "Deleting..." : "Delete"}
          cancelLabel="Cancel"
          danger={true}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 13, color: "#64748B" }}>Only admin can create, edit, or delete the master list of bookable time ranges. Duration is stored in backend minutes and used to build consultant-specific slot windows.</p>
        </div>
        <div style={{ minWidth: 320, flex: "1 1 420px", maxWidth: 520, display: "grid", gap: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8 }}>
            {[1, 2, 3].map((hours) => {
              const active = selectedDurationHours === hours;
              return (
                <button
                  key={hours}
                  type="button"
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            >
              <span>{selectedStartHour === null ? `Pick a ${selectedDurationHours}-hour slot` : formatHourRangeLabel(selectedStartHour, selectedDurationHours)}</span>
            ...
            <button
              ...
              disabled={saving}
              style={{ padding: "11px 18px", borderRadius: 10, border: "none", background: saving ? "#99F6E4" : "#0F766E", color: "#fff", fontSize: 13, fontWeight: 700, cursor: saving ? "default" : "pointer", whiteSpace: "nowrap" }}
            >
      ...
      <div style={{ background: "#fff", borderRadius: 18, border: "1px solid #E2E8F0", boxShadow: "0 10px 30px rgba(15,23,42,0.06)", overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "96px 1fr 110px 170px", gap: 12, padding: "14px 18px", background: "#F8FAFC", borderBottom: "1px solid #E2E8F0", fontSize: 12, fontWeight: 800, color: "#64748B", textTransform: "uppercase", letterSpacing: "0.06em" }}>
          <span>ID</span>
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {slots.map((slot) => (
              <div key={slot.id} style={{ display: "grid", gridTemplateColumns: "96px 1fr 110px 170px", gap: 12, padding: "14px 18px", borderBottom: "1px solid #F1F5F9", alignItems: "center" }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#0F766E" }}>#{slot.id}</span>
                <span style={{ fontSize: 14, fontWeight: 600, color: "#0F172A" }}>{slot.timeRange}</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#475569" }}>{durationMinutesToHours(slot.duration, 1)} hr</span>
                <div style={{ display: "flex", gap: 8, justifyContent: "flex-start" }}>
                  <button onClick={() => setDeleteTargetId(slot.id)} disabled={deletingId === slot.id} style={{ padding: "7px 12px", borderRadius: 8, border: "1px solid #FECACA", background: "#FEF2F2", color: "#DC2626", fontSize: 12, fontWeight: 700, cursor: deletingId === slot.id ? "default" : "pointer", opacity: deletingId === slot.id ? 0.6 : 1 }}>
      ...
      <HourRangeClockPicker
        ...
        initialDuration={selectedDurationHours}
        onClose={() => setPickerMode(null)}
        onSave={(hour, duration) => {
          setSelectedStartHour(hour);
          setSelectedDurationHours(duration);
          setPickerMode(null);
        }}
      />
    </div>
...
const AdminOffersPanel: React.FC = () => {
  ...
  const [deleteOfferTarget, setDeleteOfferTarget] = React.useState<AdminOffer | null>(null);
  ...
  React.useEffect(() => {
    getAllAdvisors()
      .then((data: any) => {
        ...
        setConsultants(arr.map((a: any) => ({ id: a.id, name: a.name || a.fullName || "Consultant" })));
      })
      .catch(() => { });
  }, []);
  ...
  return (
    <div>
      <ConfirmDialog
        ...
        title="Delete Offer?"
        message={`"${deleteOfferTarget?.title || ""}" will be permanently removed from the admin offers list.`}
        confirmLabel="Delete Offer"
        danger={true}
        busy={deleteOfferTarget ? deleting === deleteOfferTarget.id : false}
        onClose={() => { if (deleting === null) setDeleteOfferTarget(null); }}
        onConfirm={() => {
          if (!deleteOfferTarget?.id) return;
          handleDelete(deleteOfferTarget.id);
      ...
      {showForm && (
        <div style={{
          background: '#F8FAFC', border: '1.5px solid #A5F3FC', borderRadius: 16,
          padding: 24, marginBottom: 24, boxShadow: '0 2px 12px rgba(15,118,110,0.07)',
        }}>
            ...
            return (
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {offer.consultantId ? (
                    <div style={{ fontSize: 10, color: '#0F766E', fontWeight: 600, marginTop: 3, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                      <SvgIcon d={SVGS.person} size={11} color="#0F766E" strokeWidth={2.2} />
                      {consultants.find(c => c.id === offer.consultantId)?.name || "Consultant"}
                    </div>
                  ) : (
                    <div style={{ fontSize: 10, color: '#16A34A', fontWeight: 600, marginTop: 3, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                      <SvgIcon d={SVGS.globe} size={11} color="#16A34A" strokeWidth={2.1} />
                      All Consultants
                    </div>
                  )}
                </div>
...
const AnalyticsDashboard: React.FC<{ tickets: any[]; consultants: any[]; bookings: any[]; mode?: string }> = ({ tickets, consultants, bookings }) => {
  ...
  React.useEffect(() => {
    ...
    consultants.forEach((c: any) => { if (c.id) map[Number(c.id)] = c.name || c.fullName || "Consultant"; });
    ...
    try {
      ...
      if (bResult.status === "fulfilled") {
        ...
        if (arr.length > 0) {
          ...
          consultants.forEach((c: any) => {
            if (c.id) nameMap[Number(c.id)] = c.name || c.fullName || nameMap[Number(c.id)] || "Consultant";
          });
  ...
  consultants.forEach((c: any) => {
    if (c.id) consultantIdToName[Number(c.id)] = c.name || c.fullName || c.username || "Consultant";
  });
  ...
  const emptyState = (msg: string, hint: string) => (
    <div style={{ textAlign: "center", padding: "48px 20px", color: "#94A3B8" }}>
      ...
      <button onClick={doFetch} disabled={fetching}
        style={{ marginTop: 16, padding: "8px 20px", borderRadius: 9, border: "1.5px solid #A5F3FC", background: "#ECFEFF", color: "#0F766E", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0, marginTop: 4 }}>
          {fetching && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#64748B" }}>
              <div style={{ width: 14, height: 14, border: "2px solid #E2E8F0", borderTopColor: "#0F766E", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
              Fetching...
            </div>
          )}
          <button onClick={doFetch} disabled={fetching}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 9, border: "1.5px solid #E2E8F0", background: "#fff", color: "#0F766E", fontSize: 12, fontWeight: 700, cursor: fetching ? "default" : "pointer", opacity: fetching ? 0.6 : 1 }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 22 }}>
        {(["daily", "weekly", "monthly"] as const).map(r => (
          <button key={r} onClick={() => setRangeFilter(r)}
            style={{ padding: "6px 16px", borderRadius: 20, border: `1.5px solid ${rangeFilter === r ? "#0F766E" : "#E2E8F0"}`, background: rangeFilter === r ? "#ECFEFF" : "#fff", color: rangeFilter === r ? "#0F766E" : "#64748B", fontSize: 12, fontWeight: rangeFilter === r ? 700 : 500, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize" }}>
            ...
            {total === 0 ? emptyState("No tickets in this range", "Tickets created in the selected date range will appear here.") : (
              <>
                    ...
                    return (
                      <div key={d} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                        <div style={{ width: "100%", display: "flex", gap: 1, alignItems: "flex-end", height: 130 }}>
                          <div style={{ flex: 1, background: "#A5F3FC", borderRadius: "3px 3px 0 0", height: Math.max(h, v.created > 0 ? 3 : 0), transition: "height 0.3s" }} title={`${v.created} created`} />
                ...
                <div style={{ display: "flex", gap: 16, marginTop: 14, fontSize: 11, color: "#64748B" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 10, height: 10, borderRadius: 2, background: "#A5F3FC", display: "inline-block" }} /> Created</span>
      ...
      {activeTab === "agents" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, alignItems: "stretch", marginBottom: 20 }}>
            {statCard("Total Agents", agents.length, "with assigned tickets", "#0F766E")}
      ...
      {activeTab === "sla" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, alignItems: "stretch", marginBottom: 24 }}>
            ...
            {statCard("Total Tracked", slaTotal, `across all tickets`, "#0F766E")}
          </div>
      ...
      {activeTab === "revenue" && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, alignItems: "stretch", marginBottom: 24 }}>
            {statCard("Total Bookings", totalBookings, `${pendingBookings} pending / confirmed`, "#0F766E")}
            ...
            {statCard("Total Revenue", totalRevenue > 0 ? formatIndianCurrency(totalRevenue) : formatIndianCurrency(0), "from completed bookings", "#059669")}
            ...
            {topConsultants.length === 0
              ...
              : topConsultants.map((c, i) => (
                <div key={c.name} style={{ display: "grid", gridTemplateColumns: "1fr 70px 80px 80px 110px", padding: "13px 20px", borderBottom: i < topConsultants.length - 1 ? "1px solid #F8FAFC" : "none", alignItems: "center" }}
                  ...
                  <div style={{ textAlign: "center", fontSize: 15, fontWeight: 800, color: "#0F766E" }}>{c.count}</div>
                  ...
                  <div style={{ textAlign: "right", fontSize: 13, fontWeight: 700, color: c.revenue > 0 ? "#059669" : "#94A3B8" }}>
                    {c.revenue > 0 ? formatIndianNumber(c.revenue) : "--"}
                  </div>
                </div>
              ))}
          </div>
...
const ContactSubmissionsPanel: React.FC = () => {
  ...
  const [deleteTarget, setDeleteTarget] = React.useState<ContactSubmission | null>(null);
    ...
    <div>
      {deleteTarget && (
        <ConfirmDialog
          ...
          message={`Delete the contact submission from ${deleteTarget.name || deleteTarget.email}?`}
          confirmLabel="Delete"
          cancelLabel="Cancel"
          danger={true}
          onClose={() => setDeleteTarget(null)}
      ...
      {showClearConfirm && (
        <ConfirmDialog
          open={true}
          title="Clear Contact Submissions"
          message="Delete all contact submissions from local view? This cannot be undone."
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          {submissions.length > 0 && <button onClick={() => setShowClearConfirm(true)} style={{ padding: "8px 14px", borderRadius: 9, border: "1.5px solid #FECACA", background: "#FEF2F2", color: "#DC2626", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Clear All</button>}
        </div>
      </div>
                ...
                {visible.map((sub, i) => (
                  <div key={sub.id}
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      background: selected?.id === sub.id ? "#ECFEFF" : sub.read ? "#fff" : "#FAFBFF",
                      borderLeft: `3px solid ${selected?.id === sub.id ? "#0F766E" : sub.read ? "transparent" : "#0F766E"}`,
                      transition: "all 0.1s",
                    }}
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "var(--portal-profile-gradient)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ fontSize: 13, fontWeight: sub.read ? 600 : 800, color: "#0F172A", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>
                              {sub.name}
                              {!sub.read && <span style={{ marginLeft: 6, display: "inline-block", width: 7, height: 7, borderRadius: "50%", background: "#0F766E", verticalAlign: "middle" }} />}
              ...
              <div style={{ background: "var(--portal-profile-gradient)", padding: "18px 22px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#A5F3FC", marginTop: 2 }}>{selected.email}</div>
                  </div>
                ...
                <div style={{ fontSize: 11, color: "#99F6E4", marginTop: 8 }}> {fmtDate(selected.submittedAt)}</div>
              </div>
              <div style={{ padding: "20px 22px" }}>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
                  <a href={`mailto:${selected.email}?subject=Re: Your message to Meet The Masters`}
                    style={{ flex: 1, padding: "10px", borderRadius: 10, border: "none", background: "var(--color-primary-gradient)", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", textAlign: "center", textDecoration: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
...
function AdminPageInner() {
  ...
  useEffect(() => {
    const poll = async () => {
      ...
      try {
        ...
        const unread = alerts.filter((a) => !a.read && !processedNotifIds.current.has(String(a.id || a.ticketId)));
        for (const a of unread) {
          ...
          addNotification({
            type: "error",
            title: `Escalation${userName ? ` - ${userName}` : ""}`,
            message: `${userName ? `User: ${userName}. ` : ""}${message}`,
            ticketId: a.ticketId,
          });
          processedNotifIds.current.add(String(a.id || a.ticketId));
          a.read = true;
        }
        if (unread.length > 0) {
      ...
      try {
        ...
        const unreadAdmin = adminNotifs.filter((n) => !n.read && !processedNotifIds.current.has(String(n.id)));
        for (const n of unreadAdmin) {
          ...
          addNotification({
            ...
            title: n.title || `Notification${userName ? ` - ${userName}` : ""}`,
            message: `${userName ? `User: ${userName}. ` : ""}${message || ""}`,
            ticketId: n.ticketId,
          });
          processedNotifIds.current.add(String(n.id));
          n.read = true;
        }
  ...
  const fetchDashboardData = async () => {
      ...
      if (bookingsArr.length > 0 || specialBookingsArr.length > 0) {
        ...
        await Promise.all(uniqueConsultantIds.map(id =>
          (async (cid: number) => {
            ...
            consultantNameMap[cid] = "Consultant";
          })(id)
        ));
    ...
    try {
      const [adminOverview, adminCharts] = await Promise.allSettled([
        ...
        apiFetch("/analytics/admin/charts"),
      ]);
  ...
  const handleSupportAssign = (ticketId: number, agentName: string) => {
    ...
    addNotification({ type: "success", title: `Ticket Assigned`, message: `Assigned to ${agentName}.`, ticketId });
  };
  ...
  const DASHBOARD_SEARCH_ITEMS = [
    ...
    { id: "open-tickets", label: "Open Tickets", sub: ticketCount > 0 ? `${ticketCount} need attention` : "No tickets need attention", icon: "ticket" },
  ...
  const openTicketsSubtext = loading
    ? "Checking the latest support ticket status."
    : hasOpenTickets
      ? "Click to view and manage all support tickets"
      : "Support tickets that need action will appear here.";
  const openTicketsAccent = hasOpenTickets
    ? { bg: "#FFF7F7", border: "#FECACA", iconBg: "#FEE2E2", iconColor: "#EF4444", title: "#DC2626", sub: "#EF4444", cta: "#EF4444" }
    : { bg: "#F8FAFC", border: "#E2E8F0", iconBg: "#ECFEFF", iconColor: "#0F766E", title: "#0F766E", sub: "#64748B", cta: "#0F766E" };
    ...
    <div className="adm-page">
      ...
      <div className={`adm-sidebar ${isMobileMenuOpen ? "adm-sidebar-open" : ""}`}>
        <div className="adm-sidebar-logo">
          ...
          <button className="adm-close-menu-btn" onClick={() => setIsMobileMenuOpen(false)}><X size={18} /></button>
        </div>
          ...
          {navItems.map(n => (
            <button key={n.id} onClick={() => handleNavClick(n.id)}
              ...
              {n.badge != null && n.badge > 0 && (
                <span style={{ marginLeft: "auto", background: n.id === "tickets" ? "#DC2626" : "#0F766E", color: "#fff", borderRadius: 10, fontSize: 10, fontWeight: 700, padding: "1px 7px", minWidth: 18, textAlign: "center" }}>{n.badge}</span>
              )}
            </button>
          ))}
        </nav>
          ...
          <div ref={searchRef} className="adm-search-wrapper" style={{
            ...
            {searchQuery && (
              <button onClick={() => { setSearchQuery(""); setShowSearchDropdown(false); }}
                style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "#F1F5F9", border: "none", borderRadius: "50%", width: 20, height: 20, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#64748B", fontSize: 12, fontWeight: 700, lineHeight: 1, padding: 0 }}>
                <X size={11} />
              </button>
            )}
            {/* Dropdown */}
            {showSearchDropdown && searchResults.length > 0 && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, right: 0, background: "#fff", border: "1.5px solid #A5F3FC", borderRadius: 16, boxShadow: "0 18px 42px rgba(15,118,110,0.14)", zIndex: 9999, overflow: "hidden" }}>
                <div style={{ padding: "8px 14px 6px", borderBottom: "1px solid #CFFAFE", fontSize: 10, fontWeight: 700, color: "#0F766E", textTransform: "uppercase" as const, letterSpacing: "0.08em" }}>Dashboard Sections</div>
                {searchResults.map((r, i) => (
                  <div key={r.id}
                    ...
                    onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
                    <div style={{ width: 30, height: 30, borderRadius: 8, background: r.icon === "ticket" ? "#FEF3C7" : "#ECFEFF", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      {r.icon === "chart" && <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="#0F766E" strokeWidth="2"><path d="M3 3v18h18" /><path d="M7 16l4-4 4 4 4-6" strokeLinecap="round" /></svg>}
                      ...
                      {r.icon === "consultant" && <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="#0F766E" strokeWidth="2"><circle cx="9" cy="7" r="4" /><path d="M2 20c0-3.3 3.1-6 7-6s7 2.7 7 6" strokeLinecap="round" /></svg>}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#0F172A", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>{r.label}</div>
                      <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 1 }}>{r.sub}</div>
                    </div>
                    <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="#99F6E4" strokeWidth="2"><path d="M9 18l6-6-6-6" /></svg>
                  </div>
                ))}
              </div>
            )}
        ...
        <div style={{ padding: "28px 32px", flex: 1, minWidth: 0, boxSizing: "border-box", overflowY: "auto" }}>
          ...
          {activeSection === "dashboard" && (
            <>
                      ...
                      <BarChart data={bookingChartData} margin={{ top: 4, right: 16, left: -20, bottom: 0 }} barCategoryGap="35%">
                        ...
                        <Tooltip
                          ...
                          formatter={(value: any, name: any) => [
                            name === "revenue" ? formatIndianCurrency(value * 1000) : value,
                        ...
                        />
                        <Bar dataKey="bookings" fill="#0F766E" radius={[5, 5, 0, 0]} maxBarSize={32} name="bookings" />
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <Legend
                            ...
                            wrapperStyle={{ fontSize: 11, paddingTop: 8 }}
                          />
                          <Bar dataKey="bookings" fill="#0F766E" radius={[5, 5, 0, 0]} maxBarSize={40} name="bookings" />
              ...
              <div style={{ marginTop: 20 }}>
                <div className="adm-card">
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                    ...
                    <button onClick={() => setActiveSection("advisors")} style={{ background: "none", border: "none", color: "#0F766E", fontWeight: 600, fontSize: 12, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>View all <ArrowRight size={13} /></button>
                  </div>
                  {advisors.length === 0 ? (
                    ...
                    return (
                      <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: idx < Math.min(advisors.length, 4) - 1 ? "1px solid #F1F5F9" : "none" }}>
                        <div style={{ width: 38, height: 38, borderRadius: "50%", overflow: "hidden", flexShrink: 0, background: "var(--portal-profile-gradient)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          <div style={{ fontSize: 12, fontWeight: 800, color: "#F59E0B", display: "inline-flex", alignItems: "center", gap: 4 }}><StarIcon size={12} fill="#F59E0B" stroke="none" /> {a.rating.toFixed(1)}</div>
                          <div style={{ fontSize: 10, color: "#94A3B8", marginTop: 1 }}>{formatIndianCurrency(a.fee)}/session</div>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div id="dash-section-open-tickets" className="adm-card" style={{ marginTop: 20, padding: "22px 24px", background: openTicketsAccent.bg, border: `1px solid ${openTicketsAccent.border}` }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 16, minWidth: 0 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 14, background: openTicketsAccent.iconBg, color: openTicketsAccent.iconColor, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              <div id="dash-section-recent-bookings" className={`adm-card `}>
                    ...
                    <tbody>
                      {loading ? (
                        ...
                        <tr key={i} className="adm-table-row">
                          ...
                          <td className="adm-td-amount">{formatIndianCurrency(b.amount)}</td>
                        </tr>
                      )) : (
                ...
                {totalBookingsCount > 5 && (
                  <div style={{ textAlign: "center", padding: "12px 0 4px" }}>
                    <button onClick={() => setActiveSection("bookings")} style={{ background: "none", border: "none", color: "#0F766E", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>View all {totalBookingsCount} bookings <ArrowRight size={13} /></span>
                    </button>
                  </div>
                )}
              </div>
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", gap: 10 }}>
                        ...
                        <button
                          ...
                          style={{ flex: 1, padding: "11px", borderRadius: 10, border: "none", background: "#0F766E", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>View Bookings <ArrowRight size={13} /></span>
                        </button>
                      </div>
                    </>
                  ) : (
                    ...
                    <>
                      ...
                      <div style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 10, padding: "12px 14px", marginBottom: 20 }}>
                        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                          ...
                          <div style={{ fontSize: 12, color: "#475569" }}>
                            <span style={{ color: "#94A3B8", fontWeight: 600 }}>Fee: </span>{formatIndianCurrency(deleteConfirmModal.advisor.fee)}/session
                          </div>
                          <div style={{ fontSize: 12, color: "#475569" }}>
                            <span style={{ color: "#94A3B8", fontWeight: 600 }}>Rating: </span>
                            {deleteConfirmModal.advisor.rating > 0 ? <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><StarIcon size={12} fill="#F59E0B" stroke="none" /> {deleteConfirmModal.advisor.rating.toFixed(1)}</span> : "No rating"}
                          </div>
          ...
          {activeSection === "advisors" && (
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, paddingBottom: 20, borderBottom: "1px solid #E2E8F0" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, background: "var(--portal-profile-gradient)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 14px rgba(15,118,110,0.3)", flexShrink: 0 }}>
            ...
            <div>
              <div style={{ marginBottom: 24 }}>
                <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: "#0F172A", display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ width: 32, height: 32, borderRadius: 8, background: "#ECFEFF", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
                    <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#0F766E" strokeWidth="2" strokeLinecap="round"><path d="M3 3v18h18" /><path d="M7 16l4-4 4 4 4-6" strokeLinejoin="round" /></svg>
                  </span>
              ...
              {allTickets.length === 0 ? (
                <div style={{ textAlign: "center", padding: "60px 20px", background: "#F8FAFC", borderRadius: 20, color: "#94A3B8" }}>
                  ...
                  <button onClick={() => setActiveSection("tickets")} style={{ marginTop: 16, padding: "10px 24px", background: "#0F766E", color: "#fff", border: "none", borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>Go to Tickets <ArrowRight size={14} /></button>
                </div>
              ) : (
                <TicketSummaryChart tickets={allTickets} consultantNameMap={consultantNameMap} />
              )}
      ...
      <style>{`@keyframes spin { to { transform: rotate(360deg); } } @keyframes mtmPulse { 0% { transform: scale(0.80); filter: blur(3px) drop-shadow(0 0 6px rgba(15,118,110,0.20)); opacity: 0.0; } 20% { opacity: 0.6; } 50% { transform: scale(1.10); filter: blur(0px) drop-shadow(0 0 22px rgba(15,118,110,0.65)); opacity: 1.0; } 80% { opacity: 0.6; } 100% { transform: scale(0.80); filter: blur(3px) drop-shadow(0 0 6px rgba(15,118,110,0.20)); opacity: 0.0; } }`}</style>
    </div>
  );
}
