...
// ─── Shared DIO ───────────────────────────────────────────────────────────────
final _analyticsDio = ApiClient().dio;
...
class _AgentStat {
  ...
  const _AgentStat({
    required this.name, required this.assigned, required this.resolved,
    required this.totalMins, required this.resCount,
  });
}
...
class _AdminAnalyticsTabState extends State<AdminAnalyticsTab> {
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    try {
      // FIX: correct method name getFullDashboard
      final dynamic result = await _analyticsService.getFullDashboard(period: _range);
  ...
  Widget build(BuildContext context) {
    ...
    if (_data == null) {
      return const EmptyState(icon: Icons.analytics_outlined, title: 'No analytics data');
    }
      ...
      child: ListView(
        ...
        children: [
          ...
          GridView.count(
            ...
            children: [
              StatCard(title: 'Total Tickets', value: '${d.totalTickets}',
                  icon: Icons.confirmation_number_rounded, color: AppColors.primaryLight),
              StatCard(title: 'Resolved', value: '${d.resolvedTickets}',
                  icon: Icons.check_circle_outline_rounded, color: const Color(0xFF059669)),
              StatCard(title: 'SLA Breaches', value: '${d.slaBreaches}',
                  icon: Icons.timer_off_rounded, color: const Color(0xFFDC2626)),
          ...
          if (d.totalTickets > 0) ...[
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                // Mini breakdown
                Row(children: [
                  _miniStat('Open', d.totalTickets - d.resolvedTickets - (d.slaBreaches), const Color(0xFF0891B2)),
                  const SizedBox(width: 16),
                  _miniStat('Breached', d.slaBreaches, const Color(0xFFDC2626)),
                  const SizedBox(width: 16),
                  _miniStat('Resolved', d.resolvedTickets, const Color(0xFF059669)),
                ]),
              ]),
            ),
            const SizedBox(height: 16),
                            ...
                            return Padding(
                              ...
                              child: Text(
                                ...
                                style: TextStyle(
                                    ...
                                    color: getPriorityColor(priorities[v.toInt()])),
                              ),
                            );
                          }
                      ...
                      barRods: [
                        BarChartRodData(
                          ...
                          backDrawRodData: BackgroundBarChartRodData(
                            show: true,
                            toY: (d.ticketsByPriority.values.reduce(max)).toDouble(),
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 16),
              _TimeRow(label: 'Avg First Response', hours: d.avgResponseTime,
                  color: AppColors.info),
              const SizedBox(height: 12),
              _TimeRow(label: 'Avg Resolution Time', hours: d.avgResolutionTime,
  ...
  Widget _pill(String v, String label) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: _range == v ? AppColors.primaryLight : AppColors.surfaceVariant,
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: _range == v ? AppColors.primaryLight : AppColors.border),
      ),
      child: Text(label,
          style: TextStyle(
            fontSize: 12,
...
class _TicketSummaryChartState extends State<_TicketSummaryChart> {
  ...
  bool _loading = true;
  String _groupBy = 'status'; // status | priority | category
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await _analyticsDio.get('/api/tickets', queryParameters: {'size': 300});
      final raw = res.data;
      final items = raw is Map ? (raw['content'] ?? []) : (raw is List ? raw : []);
      if (mounted) setState(() { _rawTickets = items; _loading = false; });
  ...
  List<_SummaryPoint> _buildData() {
    ...
    final periods = isDaily
        ...
        : List.generate(8, (i) {
            ...
            return (
              label: '${DateFormat('d MMM').format(startD)}–${DateFormat('d MMM').format(endD)}',
              start: startD,
    ...
    String _getGroup(Map<String, dynamic> t) {
      ...
      if (_groupBy == 'priority') return (t['priority'] ?? 'Unknown').toString();
      return (t['category'] ?? 'General').toString();
    }

    return periods.map((p) {
      final groups = <String, int>{};
      for (final t in _rawTickets) {
        try {
          final created = DateTime.parse((t['createdAt'] ?? '').toString()).toLocal();
          if (created.isAfter(p.start) && created.isBefore(p.end)) {
            final g = _getGroup(t as Map<String, dynamic>);
  ...
  static const _palette = [
    Color(0xFF0F766E), Color(0xFF7C3AED), Color(0xFF059669), Color(0xFFD97706),
    Color(0xFFDC2626), Color(0xFF0891B2), Color(0xFFDB2777), Color(0xFF65A30D),
  ];
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('Tickets over time by ${_groupBy}', style: AppTextStyles.caption),
          ])),
          const Icon(Icons.bar_chart_rounded, color: AppColors.primaryLight, size: 18),
        ]),
        const SizedBox(height: 12),
        ...
        SingleChildScrollView(
          ...
          child: Row(children: [
            _controlChip('Daily', _viewMode == 'daily', () => setState(() => _viewMode = 'daily')),
            const SizedBox(width: 6),
            _controlChip('Weekly', _viewMode == 'weekly', () => setState(() => _viewMode = 'weekly')),
            const SizedBox(width: 12),
            _controlChip('Status', _groupBy == 'status', () => setState(() => _groupBy = 'status')),
            const SizedBox(width: 6),
            _controlChip('Priority', _groupBy == 'priority', () => setState(() => _groupBy = 'priority')),
            const SizedBox(width: 6),
            _controlChip('Category', _groupBy == 'category', () => setState(() => _groupBy = 'category')),
          ]),
        ),
        ...
        if (_loading)
          ...
          const SizedBox(height: 120, child: Center(
            child: Text('No ticket data yet', style: TextStyle(color: AppColors.textMuted)),
          ))
        else ...[
          // Stacked bar chart
          SizedBox(
            ...
            child: BarChart(BarChartData(
              ...
              barGroups: points.asMap().entries.map((entry) {
                ...
                if (total == 0) {
                  return BarChartGroupData(
                    x: idx,
                    barRods: [BarChartRodData(toY: 0, color: Colors.transparent, width: 16)],
                  );
                }
                double fromY = 0;
                final rods = groups.asMap().entries.map((ge) {
                  final gIdx = ge.key;
                  final g = ge.value;
                  ...
                  final rod = BarChartRodData(
                    ...
                    borderRadius: BorderRadius.vertical(
                      top: gIdx == groups.length - 1 || (gIdx == groups.indexOf(groups.lastWhere((g2) => (point.groups[g2] ?? 0) > 0, orElse: () => g)))
                          ? const Radius.circular(4)
                          : Radius.zero,
                      bottom: fromY == 0 ? const Radius.circular(4) : Radius.zero,
                    ),
                  );
                  fromY += val;
                  return rod;
                }).where((r) => r.toY > r.fromY).toList();
          ...
          // Legend
          Wrap(spacing: 10, runSpacing: 6, children: groups.asMap().entries.map((e) {
            final color = _groupBy == 'status'
                ? getStatusColor(e.value)
                : _groupBy == 'priority'
                    ? getPriorityColor(e.value)
                    : _palette[e.key % _palette.length];
            final total = points.fold<int>(0, (s, p) => s + (p.groups[e.value] ?? 0));
  ...
  Widget _controlChip(String label, bool active, VoidCallback onTap) => GestureDetector(
    onTap: onTap,
    child: AnimatedContainer(
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      for (final t in items) {
        final cat = ((t as Map<String, dynamic>)['category'] ?? 'General').toString().trim();
        if (cat.isNotEmpty) counts[cat] = (counts[cat] ?? 0) + 1;
      }
      if (mounted) setState(() { _counts.clear(); _counts.addAll(counts); _loading = false; });
    } catch (_) { if (mounted) setState(() => _loading = false); }
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...top.asMap().entries.map((e) {
          final color = const [
            Color(0xFF0F766E), Color(0xFF7C3AED), Color(0xFF059669),
          ...
          return Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(e.value.key, style: AppTextStyles.label, overflow: TextOverflow.ellipsis)),
                Text('${e.value.value}',
...
class _AgentPerformanceSection extends StatefulWidget {
  ...
  @override
  State<_AgentPerformanceSection> createState() => _AgentPerformanceSectionState();
}
...
class _AgentPerformanceSectionState extends State<_AgentPerformanceSection> {
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    try {
      final res = await _analyticsDio.get('/api/tickets', queryParameters: {'size': 200});
      final raw = res.data;
      final items = raw is Map ? (raw['content'] ?? []) : (raw is List ? raw : []);
      final map = <String, _AgentStat>{};
      for (final t in items) {
        final name =
            (t as Map<String, dynamic>)['agentName']?.toString() ??
            t['consultantName']?.toString();
        if (name == null || name.isEmpty) continue;
        final cur = map[name] ??
            _AgentStat(name: name, assigned: 0, resolved: 0, totalMins: 0, resCount: 0);
      ...
      if (mounted) setState(() {
        _stats = map.values.toList()..sort((a, b) => b.assigned.compareTo(a.assigned));
        _loading = false;
      });
    } catch (_) { if (mounted) setState(() => _loading = false); }
  }
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (_loading)
        ...
        const Center(child: Padding(
          ...
          child: Text('No agent data yet', style: TextStyle(color: AppColors.textMuted)),
        ))
      else ...[
        // Table header
        Container(
          ...
          child: const Row(children: [
            ...
            Expanded(flex: 1, child: Text('RATE', textAlign: TextAlign.center, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: AppColors.textSecondary, letterSpacing: 0.5))),
        ...
        ..._stats.map((s) {
          ...
          final avgStr = avgMin > 0
              ? (avgMin >= 60 ? '${(avgMin / 60).toStringAsFixed(1)}h' : '${avgMin}m')
              : '—';
          ...
          return Padding(
            ...
            child: Row(children: [
              Expanded(flex: 3, child: Row(children: [
                ...
                const SizedBox(width: 8),
                Expanded(child: Text(s.name, style: AppTextStyles.label, overflow: TextOverflow.ellipsis)),
              ])),
              Expanded(flex: 1, child: Text('${s.assigned}',
                  textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13))),
              Expanded(flex: 1, child: Text('${s.resolved}',
                  textAlign: TextAlign.center,
                  style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: rateColor))),
              Expanded(flex: 1, child: Text('$rate%',
                  textAlign: TextAlign.center,
                  style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: rateColor))),
              Expanded(flex: 2, child: Text(avgStr,
...
class _ChartCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (legend != null) ...[
        const SizedBox(height: 12),
        Wrap(spacing: 12, runSpacing: 6, children: legend!.map((l) => Row(
          mainAxisSize: MainAxisSize.min,
          children: [
