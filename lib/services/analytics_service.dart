...
class AnalyticsService {
  ...
  Future<List<Map<String, dynamic>>> getAnalyticsTicketsAll({
    ...
    final endpoints = <Future<dynamic> Function()>[
      () => _apiClient.dio.get('/api/tickets',
            queryParameters: {'page': 0, 'size': fallbackSize, 'sort': 'createdAt,DESC'}),
      () => _apiClient.dio.get('/api/tickets',
            queryParameters: {'page': 0, 'size': fallbackSize}),
      () => _apiClient.dio.get('/api/analytics/tickets/all'),
      () => _apiClient.dio.get('/analytics/tickets/all'),
      () => _apiClient.dio.get('/api/analytics/tickets',
            queryParameters: {'page': 0, 'size': fallbackSize}),
    ];
    ...
    for (final ep in endpoints) {
      try {
        ...
        if (totalPages > 1) {
          final sz = _toInt(root['size'] ?? root['pageSize']);
          final pageSize = sz > 0 ? sz : fallbackSize;
          final all = <Map<String, dynamic>>[...items];
          for (var page = 1; page < totalPages && page < 20; page++) {
            try {
              final next = await _apiClient.dio.get('/api/tickets',
                  queryParameters: {'page': page, 'size': pageSize, 'sort': 'createdAt,DESC'});
              all.addAll(_asMapList(next.data));
            } catch (_) {}
          }
          final seen = <String>{};
          final deduped = all.where((row) {
            final key = (row['id'] ?? row['ticketId'] ?? '').toString().trim();
  ...
  /// Tries multiple analytics endpoints for production compatibility.
  Future<Map<String, dynamic>?> getAnalytics({
    ...
    for (final ep in eps) {
      try {
        final resp = await _apiClient.dio.get(ep,
            queryParameters: {'period': period});
        final map = _unwrapMap(resp.data, preferredKeys: const ['analytics']);
